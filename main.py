...
def send_event_to_server(event_id):
    try:
        url = f"{MAIN_API}/v1/events/check-in/{event_id}"
        r = requests.post(url, timeout=5)
        return r.json()
    except Exception as e:
        return {"status": "error", "message": str(e)}
...
def event_check_in(event_id):
    ...
    return render_template("event.html", event_id=event_id, event=result)

@app.route("/api/tickets/check-in/<ticket_id>", methods=["POST"])
def ticket_check_in(ticket_id):
    app.logger.info(f"[TICKET] check-in request: {ticket_id}")

    result = send_ticket_to_server(ticket_id)

    if result.get("status") == "error":
        app.logger.error(f"[TICKET] failed: {result}")
    else:
